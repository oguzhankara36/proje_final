...
class boga_kadin extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        resizeToAvoidBottomInset: false,
        appBar: AppBar(
          title: Text('BOĞA KADINI'),
        ),
        body: bogaa_kadin());
  }
}

class bogaa_kadin extends StatefulWidget {
  @override
  _bogaa_kadinState createState() => _bogaa_kadinState();
}

class _bogaa_kadinState extends State<bogaa_kadin> {
  var gelenyazi="";
  Artboard _artboard;
  ...
  void _rivedosyasi() async {
    final bogadosya = await rootBundle.load('assets/boga_kadın.riv');
  ...
  void _animasyon2() async {
    _artboard.addController(
      _animasyonkontrol2 = SimpleAnimation('boga_kadın'),
    );
    setState(() => _animasyonkontrol2.isActive = true);
  }
  yazigetir() {
    FirebaseFirestore.instance.collection("burclar")
        .doc('bogakadın').get().then((gelen) {
      setState(() {
        gelenyazi = gelen.data()['bogakadın'];
      });
    }
    );
  }
