...
class balik_erkek extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      resizeToAvoidBottomInset: false,
        appBar: AppBar(
          title: Text('BALIK ERKEĞİ'),
        ),
        body: balikk_erkek());
  }
}

class balikk_erkek extends StatefulWidget {
  @override
  _balikk_erkekState createState() => _balikk_erkekState();
}

class _balikk_erkekState extends State<balikk_erkek> {
  var gelenyazi="";
  Artboard _artboard;
  ...
  void _rivedosyasi() async {
    final balikdosya = await rootBundle.load('assets/balikburcu.riv');
  ...
  void _animasyon2() async {
    _artboard.addController(
      _animasyonkontrol2 = SimpleAnimation('Loop'),
    );
    setState(() => _animasyonkontrol2.isActive = true);
  }
  yazigetir() {
    FirebaseFirestore.instance.collection("burclar")
        .doc('balık').get().then((gelen) {
      setState(() {
        gelenyazi = gelen.data()['balikerkek'];
      });
    }
    );
  }
