...
class yay_erkek extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        resizeToAvoidBottomInset: false,
        appBar: AppBar(
          title: Text('YAY ERKEĞİ'),
        ),
        body: yayy_erkek());
  }
}

class yayy_erkek extends StatefulWidget {
  @override
  _yayy_erkekState createState() => _yayy_erkekState();
}

class _yayy_erkekState extends State<yayy_erkek> {
  var gelenyazi="";
  Artboard _artboard;
  ...
  void _rivedosyasi() async {
    final yaydosya = await rootBundle.load('assets/yayerkek.riv');
  ...
  void _animasyon2() async {
    _artboard.addController(
      _animasyonkontrol2 = SimpleAnimation('yayerkek'),
    );
    setState(() => _animasyonkontrol2.isActive = true);
  }
  yazigetir() {
    FirebaseFirestore.instance.collection("burclar")
        .doc('yayerkek').get().then((gelen) {
      setState(() {
        gelenyazi = gelen.data()['yayerkek'];
      });
    }
    );
  }
