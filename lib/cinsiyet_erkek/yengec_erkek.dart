...
class yengec_erkek extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        resizeToAvoidBottomInset: false,
        appBar: AppBar(
          title: Text('YENGEÇ ERKEĞİ'),
        ),
        body: yengecc_erkek());
  }
}

class yengecc_erkek extends StatefulWidget {
  @override
  _yengecc_erkekState createState() => _yengecc_erkekState();
}

class _yengecc_erkekState extends State<yengecc_erkek> {
  var gelenyazi="";
  Artboard _artboard;
  ...
  void _rivedosyasi() async {
    final yengecdosya = await rootBundle.load('assets/yengecerkek.riv');
  ...
  void _animasyon2() async {
    _artboard.addController(
      _animasyonkontrol2 = SimpleAnimation('yengecerkek'),
    );
    setState(() => _animasyonkontrol2.isActive = true);
  }
  yazigetir() {
    FirebaseFirestore.instance.collection("burclar")
        .doc('yengecerkek').get().then((gelen) {
      setState(() {
        gelenyazi = gelen.data()['yengecerkek'];
      });
    }
    );
  }
